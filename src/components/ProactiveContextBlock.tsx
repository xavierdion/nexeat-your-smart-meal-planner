  ...
  if (props.variant === "inline") {
    return (
      <div className="flex gap-2 items-start bg-[#FEF0ED] rounded-[10px] px-3 py-2.5">
        <Calendar size={14} className="text-[#E07A5F] shrink-0 mt-0.5" />
        <span className="text-[12px] text-[#E07A5F] font-medium leading-[1.4]">
          {props.text}
        </span>
      </div>
    );
  ...
  return (
    <div className="bg-surface-cool rounded-2xl p-4 flex flex-col gap-3">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-eyebrow uppercase text-[#E07A5F]">
          Contexte calendrier
        </span>
      </div>
      ...
      <div className="flex items-stretch gap-0">
        <div className="flex-1 bg-white rounded-xl px-3 py-2.5">
          <p className="text-eyebrow uppercase text-[#2A2D35]/50">
            {mealLabel} · {mealTime}
          </p>
          <p className="font-display text-display-sm text-[#2A2D35] mt-0.5">
            Repas optimisé
          </p>
        </div>

        <div className="flex items-center justify-center px-1.5">
          <div className="relative w-6 h-[2px] bg-[#E07A5F]/40">
            <ArrowRight
              size={12}
              className="text-[#E07A5F] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
            />
          </div>
        </div>
        ...
        >
          ...
          <p className="font-display text-display-sm text-[#2A2D35] line-clamp-1 mt-0.5">
            {eventTitle}
          </p>
        </div>
      </div>
      ...
      {/* Rationale */}
      <p className="text-[13px] text-[#2A2D35]/70 leading-relaxed">{rationale}</p>

      {/* Footer */}
      <p className="text-[10px] text-[#2A2D35]/40">
